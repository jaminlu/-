...
function wget_app()
{
        mkdir $BASE/qat1.7
        echo $BASE/qat1.7
        wget -c http://xxx.net/nginx/qat1.7.l.4.1.0-00022.tar.gz -O - |tar -zx -C $BASE/qat1.7 \
        && wget -c https://www.openssl.org/source/openssl-1.1.0f.tar.gz -O - |tar -zx \
        &&  wget -c https://github.com/intel/QAT_Engine/archive/v0.5.36.tar.gz -O - |tar -zx 
}
...
{
        cd $BASE \
        && tar zxvf v0.5.36.tar.gz \
        && cd QAT_Engine-0.5.36 \
...
function restart_qat_service()
{
        ...
        && cp -f $BASE/QAT_Engine-0.5.36/qat/config/dh895xcc/multi_process_optimized/dh895xcc_qa_dev0.conf /etc/dh895xcc_dev0.conf \
        && cp -f $BASE/QAT_Engine-0.5.36/qat/config/dh895xcc/multi_process_optimized/dh895xcc_qa_dev0.conf /etc/dh895xcc_dev1.conf \
...
{
        /usr/local/ssl/bin/openssl speed -engine qat -elapsed -multi 128 -async_jobs 64 rsa2048
