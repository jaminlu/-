#define NGX_CONFIGURE " --prefix=/usr/local/nginx --without-http_rewrite_module --with-http_ssl_module --with-http_stub_status_module --with-http_v2_module --add-dynamic-module=modules/nginx_qat_module --with-cc-opt='-DNGX_SECURE_MEM -I/usr/local/ssl/include -Wno-error=deprecated-declarations' --with-ld-opt='-Wl,-rpath=/usr/local/ssl/lib -L/usr/local/ssl/lib'"
